...
class RoutingServer:
    ...
    def handle_activation_from_endpoint(
        ...
        try:
            ...
            if (
                    act.previous_router_ip  # 有旧路由的 IP
                    and act.previous_router_id  # 有旧路由的 ID
                    and act.previous_router_id != my_id  # 且旧路由 != 自己
            ):
                ...
                bind_port = self.config['server']['bind_port']
                self.udp_socket.sendto(reply, (act.previous_router_ip, bind_port))
                self.logger.info(
                    f"[ACT] sent deactivation of {act.endpoint_id} to "
                    f"{act.previous_router_id}@[{act.previous_router_ip}]:{bind_port}"
                )
