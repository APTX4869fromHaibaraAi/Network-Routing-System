...
class TunnelConnection:
    def __init__(
        ...
        self.rtt_ms = 0
        self.last_activation_ack_time = 0.0
    ...
    def receive(self, timeout: float = 0.1) -> Optional[bytes]:
        ...
        try:
            ...
            if packet.msg_type == MessageType.ACTIVATION_ACK:
                self.last_activation_ack_time = time.time()
                logging.info(f"[ACT-ACK] received from {self.router_info.router_id}")
                return None
    ...
    def close(self):
        ...
        if self.socket:
            try:
                ...
                pass
        # 把 socket 引用清掉，这样即使别的地方没检查 connected，也不会再拿它去 select
        self.socket = None
...
class NetworkAccessEndpoint:
    ...
    def _on_wan_state_event(self, ifname: str, is_up: bool):
        ...
        - 对 UP 事件只更新状态并打印日志，真正的隧道重连改为由 IPv6 地址事件触发。
        """
        with self.tunnel_lock:
            ...
            # 走到这里说明是 UP 事件：只记录状态，等待 IPv6 地址事件来驱动重连
            self.logger.info(
                f"[WAN-EVENT] interface {ifname} UP, "
                f"waiting for IPv6 address event to reconnect tunnels"
            )

    def _on_wan_ipv6_acquired(self, ifname: str):
        """
        当检测到某个 WAN 接口获得（或更新）全局 IPv6 地址时调用：
        - 刷新 self.wan_interfaces 中该接口的 ipv6 字段
        - 收集该 WAN 上所有未连接的隧道，基于新的 IPv6 进行重连
        """
        reconnect_tunnels = []
        bind_ipv6 = None
        ...
        with self.tunnel_lock:
            ...
            # 只关注 scope global 的 IPv6 地址
            bind_ipv6 = self.get_interface_ipv6(ifname)
            if not bind_ipv6:
                # IPv6 地址事件触发了，但还没有全局 IPv6（可能先有 link-local）
                self.logger.info(
                    f"[WAN-EVENT] IPv6 address event on {ifname} but no global IPv6 yet"
                )
                return
        ...
        if not reconnect_tunnels:
            self.logger.info(
                f"[WAN-EVENT] IPv6 acquired on {ifname} but no tunnels to reconnect"
            )
            return
    ...
    def wan_event_monitor_loop(self):
        """
        使用内核 Netlink 监听 WAN 接口的事件：
        - RTM_NEWLINK / RTM_DELLINK 处理链路 UP/DOWN
        - RTM_NEWADDR 处理 IPv6 地址获取事件（用来触发隧道重连）
        ...
        # 这些值和内核头文件一致：
        #   include/uapi/linux/rtnetlink.h / if_link.h / if_addr.h
        RTM_NEWLINK = 16
        RTM_DELLINK = 17
        RTM_NEWADDR = 20
        RTM_DELADDR = 21  # 目前暂时不用，但保留常量
        IFLA_IFNAME = 3
        RTMGRP_LINK = 1
        RTMGRP_IPV6_IFADDR = 0x100  # 监听 IPv6 地址事件
        ...
        try:
            # 同时订阅 LINK 和 IPv6 地址事件
            groups = RTMGRP_LINK | RTMGRP_IPV6_IFADDR
            nl_sock.bind((os.getpid(), groups))
        ...
        self.logger.info(
            f"[WAN-EVENT] started netlink monitor for: {', '.join(sorted(wan_names))}"
        )
                    ...
                    if nlmsg_type in (RTM_NEWLINK, RTM_DELLINK):
                        ...
                        if len(payload) >= 8:
                            ...
                            if ifa_family == socket.AF_INET6:
                                try:
                                    ifname = socket.if_indextoname(ifa_index)
                                except OSError:
                                    ifname = None

                                if ifname and ifname in wan_names:
                                    self.logger.info(
                                        f"[WAN-EVENT] RTM_NEWADDR (IPv6) on {ifname}, "
                                        f"trigger reconnect check"
                                    )
                                    # 触发一次 IPv6 获取事件的处理逻辑
                                    self._on_wan_ipv6_acquired(ifname)

                    # 处理完一条 netlink 消息，跳到下一条
    ...
    def send_activation(self, previous_router: Optional[RouterInfo] = None) -> bool:
        """
        向当前 active_tunnel 对应的路由器发送 EndpointActivation 激活包，
        并等待路由器回 ACTIVATION_ACK。若在超时时间内未收到 ACK，则重试多次。

        previous_router:
            - None：首次上线，没有旧路由器
            - RouterInfo：从这台旧路由器切过来（告知新路由器去通知旧路由器清理路由）
        """
        # 先拿一份当前 active_tunnel 的快照，避免在等待 ACK 过程中被切换
        ...
        max_retries = tunnel_cfg.get("activation_max_retries", 100)
        ack_timeout = tunnel_cfg.get("activation_ack_timeout", 3.0)  # 每次等待 ACK 的时间

        router_id = tunnel.router_info.router_id
        self.logger.info(
            f"[ACT] sending activation to router {router_id}, "
            f"previous_router={prev_id}, "
            f"max_retries={max_retries}, ack_timeout={ack_timeout}s"
        )
        ...
        for attempt in range(1, max_retries + 1):
            # 每次尝试前先清掉旧的 ACK 时间
            tunnel.last_activation_ack_time = 0.0
            ...
            try:
                ...
                ok = False
                self.logger.error(
                    f"[ACT] failed to send activation to {router_id} "
            ...
            self.logger.info(
                f"[ACT] activation sent to {router_id} "
                f"(attempt {attempt}/{max_retries})"
            )

            # 等待 ACK：在 ack_timeout 时间窗口内轮询 last_activation_ack_time
            start_wait = time.time()
            while time.time() - start_wait < ack_timeout:
                if tunnel.last_activation_ack_time > 0:
                    self.logger.info(
                        f"[ACT] activation ACK received from {router_id} "
                        f"after attempt {attempt}"
                    )
                    success = True
                    break
                time.sleep(0.05)  # 50ms 间隔，避免忙等

            if success:
                break

            self.logger.warning(
                f"[ACT] no ACK from {router_id} within {ack_timeout}s "
                f"(attempt {attempt}/{max_retries})"
            )

        if not success:
            self.logger.error(
                f"[ACT] activation to {router_id} failed: "
                f"no ACK after {max_retries} attempts"
            )
    ...
    def receive_from_tunnels(self):
        ...
        while self.running:
            try:
                ...
                valid_sockets = []
                # 为了能在循环中删掉无效 socket，把 items() 先 list 一下
                for sock, tunnel in list(socket_map.items()):
                    try:
                        fd = sock.fileno()
                    except OSError:
                        fd = -1

                    if fd < 0 or not tunnel.connected:
                        # 这个 socket 已经关闭 / tunnel 已不再连接，从 map 里移除
                        socket_map.pop(sock, None)
                        continue
    ...
    def start(self):
        ...
        # === 3) 建立隧道：失败则重试 ===
        while self.running:
            # 每次重试前刷新一次 WAN IPv6，防止 5G 场景下接口先 UP 再拿 IPv6
            self._init_wan_addresses()
