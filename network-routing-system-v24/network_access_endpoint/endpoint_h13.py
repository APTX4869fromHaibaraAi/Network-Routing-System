...
class NetworkAccessEndpoint:
    ...
    def wan_event_monitor_loop(self):
        """
        使用 'ip monitor link' 监听 WAN 接口的 UP/DOWN 事件。
        没有任何 sleep 轮询，只在有内核事件时被唤醒。
        """
        wan_names = {
            wan.get("name")
            for wan in (self.wan_interfaces or [])
        ...
        if not wan_names:
            self.logger.info("[WAN-EVENT] no WAN interfaces configured, monitor thread exit")
            return
        ...
        try:
            ...
            self.wan_monitor_proc = proc
        except Exception as e:
            self.logger.error(
                f"[WAN-EVENT] failed to start 'ip monitor link': {e}"
            )
            return

        self.logger.info(
            f"[WAN-EVENT] started 'ip monitor link' for: {', '.join(sorted(wan_names))}"
        )

        while self.running and proc.poll() is None:
            line = proc.stdout.readline()
            if not line:
                # 阻塞读：这里一般不会 busy loop，不额外 sleep
                continue
            ...
            try:
                ...
                raw_ifname = parts[0]
                # 典型形态: "wan0:" 或 "wan0@if3:"
                ifname = raw_ifname.rstrip(":")
                if "@" in ifname:
                    ifname = ifname.split("@", 1)[0]

                if ifname not in wan_names:
                    # 不是我们关心的业务 WAN，忽略
                    continue
                ...
                if "state DOWN" in text:
                    is_up = False
                elif "state UP" in text or "UP," in text or "UP>" in text:
                    is_up = True
                else:
                    # 实在看不出来，就兜底再测一次
                ...
                self.logger.error(
                    f"[WAN-EVENT] error parsing line from 'ip monitor link': {line!r}, err={e}"
                )

        self.logger.info("[WAN-EVENT] wan_event_monitor_loop exiting")
