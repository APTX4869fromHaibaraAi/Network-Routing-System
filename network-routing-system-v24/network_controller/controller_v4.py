...
class NetworkController:
    ...
    def register_with_load_balancer(self):
        """在启动时向负载均衡器注册控制器 IP"""
        lb_conf = self.config.get('load_balancer', {})
        lb_domain = lb_conf.get('domain')
        lb_port = lb_conf.get('port', 8080)
        controller_id = lb_conf.get('controller_id', 'controller-1')

        if not lb_domain:
            self.logger.warning("No load_balancer.domain in config; skip LB registration")
            return

        # 1) DNS 解析负载均衡器域名
        try:
            lb_ip = socket.gethostbyname(lb_domain)
            self.logger.info(f"Resolved load balancer {lb_domain} -> {lb_ip}")
        except Exception as e:
            self.logger.error(f"Failed to resolve load balancer domain {lb_domain}: {e}")
            return

        # 2) 确定要上报的控制器 IP
        advertise_ip = self.config['controller'].get('advertise_ip')
        url = f"http://{lb_ip}:{lb_port}/register_controller"
        payload = {
        ...
        while True:
            try:
                ...
                if resp.status_code == 200:
                    self.logger.info(
                        f"Registered controller to LB {lb_domain}({lb_ip}) successfully: {payload}"
                    )
                    break  # 成功就退出重试
                else:
                    self.logger.error(
                        f"Failed to register controller to LB: status={resp.status_code}, body={resp.text}"
                    )
            except requests.exceptions.RequestException as e:
                self.logger.error(f"Error registering controller with LB: {e}")
            time.sleep(1)
...
def main():
    ...
    controller.register_with_load_balancer()
